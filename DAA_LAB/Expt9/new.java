...
class Main {
    ...
    public static void solve(Item arr[]) {
        ...
        current.tv = current.tw = current.ub = current.lb = current.level = 0;
        current.flag = false;
        ...
        while (!pq.isEmpty()) {
            current = pq.poll();
            System.out.println(current.tv + " " + current.tw);
            ...
            if (current.tw + arr[current.level].weight <= capacity) {
                left.ub = upperBound(current.tv - arr[level].value, current.tw + arr[level].weight, level + 1,
                        arr);
                left.lb = lowerBound(current.tv - arr[level].value, current.tw + arr[level].weight, level +
                        1, arr);
